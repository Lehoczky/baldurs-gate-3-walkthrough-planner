  ...
  >
    ...
    <div class="my-1 flex flex-1 flex-col">
      <div class="flex flex-1 flex-col">
        ...
        <div class="text-opacity-90 mb-1 text-sm leading-4 text-surface-200">
          {{ location.description }}.
        </div>
      </div>

      <div class="text-opacity-80 text-sm text-primary-300">
        <a
          :href="location.act.link"
          target="_blank"
          class="underline-offset-2 hover:underline"
          >{{ location.act.name }}</a
        >,
        <a
          v-if="location.area.link"
          :href="location.area.link"
          target="_blank"
          class="underline-offset-2 hover:underline"
          >{{ location.area.name }}</a
        ><span v-else>{{ location.area.name }}</span>
      </div>
    </div>
  </div>
</template>
